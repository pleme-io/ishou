//! Borealis render targets — the stylix base16/base24 schemes + an SVG
//! palette preview, all sourced from the BORN `BorealisPalette` tokens.
//!
//! Unlike the legacy `stylix` renderer (which maps the Nord `TokenSet`),
//! these targets construct `BorealisPalette::night()` internally — the
//! Borealis tokens are not part of the Nord `TokenSet`, they are their
//! own BORN source. The base16/base24 YAML is emitted through
//! `serde_yaml` over an `IndexMap` (spec §3) — TYPED EMISSION, no
...
use ishou_tokens::{BorealisPalette, Rgb};
use serde::Serialize;
...
const HEADER: &str =
    "# Generated by ishou-render::borealis — DO NOT EDIT\n\
     # Source of truth: pleme-io/ishou/crates/ishou-tokens/src/borealis.rs\n\
     # Spec: BOREALIS fleet theme spec v2 (judge-approved)\n";

/// Render the Borealis **base16** stylix scheme YAML.
///
/// Pure — `BorealisPalette::night()` is deterministic, so this is too.
/// serde_yaml owns the escaping; the slot order is the spec §3 order.
#[must_use]
...
/// Render the Borealis **base24** stylix scheme YAML (base16 + the
...
pub fn render_svg_palette() -> String {
    ...
    for (i, (name, rgb)) in entries.iter().enumerate() {
        ...
        let text_fill = if lum > 140.0 { "#1F222F" } else { "#F5F7FA" };
        chips.push_str(&format!(
            "  <rect x=\"{x}\" y=\"{y}\" width=\"{chip}\" height=\"{chip}\" rx=\"8\" fill=\"{hex}\"/>\n  \
             ...
             <text x=\"{x}\" y=\"{ly}\" font-family=\"monospace\" font-size=\"10\" fill=\"#D4D9E3\">{name}</text>\n",
            tx = x + 6,
            ty = y + chip - 8,
            ly = y + chip + 15,
        ));
    ...
    format!(
        "<!-- ishou Borealis palette preview (generated) -->\n\
         <svg xmlns=\"http://www.w3.org/2000/svg\" viewBox=\"0 0 {width} {height}\" width=\"{width}\" height=\"{height}\">\n  \
         <rect width=\"{width}\" height=\"{height}\" fill=\"#1F222F\"/>\n{chips}</svg>\n"
    )
}
...
mod tests {
    ...
    fn base16_has_all_16_slots_lowercase_unprefixed() {
        ...
        // Spec §3 — base00 is night0 (#1F222F → 1f222f), unprefixed lc.
        assert!(out.contains("1f222f"), "base00 night0 missing:\n{out}");
    ...
    fn base24_has_the_real_brights() {
        ...
        // base14 = green_bright (#74E29F), base12 = red_bright.
        assert!(out.contains("base14: 74e29f"), "base14 green_bright:\n{out}");
        assert!(out.contains("base12: ff8482"), "base12 red_bright:\n{out}");
